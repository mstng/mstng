...
PHASE 8では**ログアウト前のコピーを再送**し、Sessionは401、JWTは期限内なら200になる違いを実測する。
即時失効と複数サーバーのトレードオフは、実測と区別した説明モデルで考える。
PHASE 9では**Access / Refresh Tokenの期限切れ・更新・ローテーション・再利用検知**を実測。
